...
class SubstackClient:
    ...
    def get_recent_posts(self, since_hours: int = 24) -> list[dict]:
        """Fetch recent posts from all subscribed publications.
        ...
        for pub in publications:
            ...
            try:
                ...
                resp = self.session.get(url, params={"sort": "new", "offset": 0, "limit": 10})
                resp.raise_for_status()
                posts = resp.json()
                ...
                logger.info(f"  {pub_name}: {count} new post(s)")
            except requests.RequestException as e:
                logger.warning(f"  {pub_name}: failed to fetch archive ({e})")
