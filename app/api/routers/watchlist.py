...
from app.api.schemas.watchlist import WatchlistEntryOut, WatchlistEntryUpdate, WatchlistResponse
from app.api.services.price_service import refresh_all_prices
from app.api.services.watchlist_service import list_watchlist
...
def _serialize_entry(e) -> dict:
    price_change_pct = None
    if e.price_at_mention and e.current_price and e.price_at_mention > 0:
        price_change_pct = round((e.current_price - e.price_at_mention) / e.price_at_mention * 100, 2)
...
async def api_list_watchlist(
    sort_by: str = Query("mention_date", pattern="^(mention_date|ticker|company|current_price|price_at_mention)$"),
