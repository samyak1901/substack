...
from app.api.schemas.digests import DigestDetail, DigestListResponse, DigestSummary
from app.api.services.digest_service import get_digest_with_articles, get_latest_digest, list_digests
from app.dependencies import get_db

router = APIRouter(prefix="/api/digests", tags=["digests"])


def _serialize_digest_summary(d) -> dict:
    return {
        "id": d.id,
        "date": d.date,
        "overview": d.overview,
...
def _serialize_digest_detail(d) -> dict:
    return {
        **_serialize_digest_summary(d),
        "articles": [
            {
                "id": a.id,
                "title": a.title,
...
async def api_list_digests(
    ...
    return DigestListResponse(
        digests=[DigestSummary(**_serialize_digest_summary(d)) for d in digests],
        total=total,
        page=page,
        page_size=page_size,
    )
...
async def api_latest_digest(db: AsyncSession = Depends(get_db)):
    ...
    if not d:
        return None
    return DigestDetail(**_serialize_digest_detail(d))
